...
1. Harvest and cheap pre-filter job leads with Codex Browser Use / `browser-use`, or Chrome fallback when needed.
...
2. Use Codex Browser Use / `browser-use`, or Chrome fallback when needed, to inspect the live listing and application questions.
...
- Browser automation prefers Codex Browser Use / `browser-use`, with Chrome as fallback for signed-in or Browser-blocked flows.
